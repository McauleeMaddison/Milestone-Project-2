...
| As a collector, I want to preview available packs before choosing one so I can understand what each pack contains. | The Store displays Crypto, Cyber and Alien pack previews. Clicking a pack opens it to reveal three actual pups from that collection, with keyboard activation working via the native button focus and Enter/Space controls. | ![NFTPups Store page showing an interactive pack preview flipped to reveal its contents](assets/images/screenshots/store-user-story.jpg) |
...
| Store card keyboard interaction | Open the Store and use `Tab` to focus each pack preview without using the mouse. | Each native Store button receives a visible focus state, and pressing Enter or Space opens the pack to reveal three matching card previews. | Pass |
